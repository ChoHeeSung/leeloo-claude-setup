...
- **HISTORY.md** (프로젝트 루트): 날짜/시/분 + 작업 요약 + 상세 파일 참조 경로만 기록한다.
  ```
  | 날짜 | 작업 | 상세 |
  | 2026-03-31 14:30 | 작업 요약 한 줄 | [상세](history/2026-03-31_1430_제목.md) |
  ```
- **history/ 폴더**: 세부 내용(지시 요약, 작업 내용, 결과, 코드 스니펫 등)은 개별 파일로 저장한다.
  - 파일명 형식: `{YYYY-MM-DD}_{HHMM}_{제목}.md`
  - 코드를 작성한 경우, 핵심 코드 스니펫을 포함하여 변경/추가 내역을 추적할 수 있게 한다.
...
| `/lk-code-review [code\|plan] [path]` | Gemini+Claude 이중 리뷰 |
| `/lk-todo` | TODO 리스트 관리 |
...
### 워크플로우 흐름
```
/lk-plan → /lk-plan-cross-review → /lk-pdca design → /lk-pdca do
→ /lk-pdca analyze → (Match Rate ≥90%) → /lk-pdca report
                    → (Match Rate <90%) → 자동 개선 반복
```

### 교차검증
- Plan/Design 완료 후 `/lk-plan-cross-review`로 Gemini 독립 검증
