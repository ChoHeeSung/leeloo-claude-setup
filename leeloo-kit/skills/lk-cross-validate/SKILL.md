---
name: lk-cross-validate
description: "gemini-cli로 Plan/Design을 교차검증합니다. Score Card + 메트릭 저장. /lk-cross-validate [file-path]"
user_invocable: true
argument-hint: "[file-path]"
---

# /lk-cross-validate — Gemini 교차검증 (Score Card)

이 스킬은 Claude가 작성한 Plan/Design 문서를 gemini-cli를 통해 독립적으로 교차검증하고,
Score Card를 파싱하여 메트릭을 저장합니다. 반복 검증 시 이전 점수와 비교합니다.
...
2. 파일이 없으면:
   ...
   Usage: /lk-cross-validate [file-path]
   ```
   중단.
3. 파일이 1개면 해당 파일을 사용
...
수정 후 다시 /lk-cross-validate {파일경로} 를 실행하세요.
```
