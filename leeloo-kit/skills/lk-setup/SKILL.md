...
/lk-setup plugins install-context-mode — register context-mode marketplace + install plugin
...
- `plugins install-context-mode` → **plugins install-context-mode** action
...
# context-mode marketplace registration
grep -q '"context-mode"' ~/.claude/settings.json 2>/dev/null && echo "REGISTERED" || echo "NOT_REGISTERED"

# context-mode plugin installation
grep -q 'context-mode@context-mode' ~/.claude/settings.json 2>/dev/null && echo "INSTALLED" || echo "NOT_INSTALLED"
...
| context-mode 마켓플레이스 | ✅ 등록됨 / ❌ 미등록 | /lk-setup plugins install-context-mode |
| context-mode 플러그인 | ✅ 설치됨 / ❌ 미설치 | /lk-setup plugins install-context-mode |
...
#### plugins install-context-mode

Register the context-mode marketplace + install the context-mode plugin.
context-mode routes large tool output through a sandboxed subprocess, so only summaries enter the main context window — major token savings on log/test/git/API workloads.
...
2. **If marketplace not registered**: Read `~/.claude/settings.json`, then Edit to add the following entry to `extraKnownMarketplaces`:
   ...
   ```
   - Already registered: "context-mode 마켓플레이스가 이미 등록되어 있습니다."

3. **Plugin install check**: Bash:
   ```bash
   grep -q 'context-mode@context-mode' ~/.claude/settings.json 2>/dev/null && echo "INSTALLED" || echo "NOT_INSTALLED"
   ```

4. **If plugin missing**: Edit to add to `enabledPlugins` in `~/.claude/settings.json`:
   ```json
   "context-mode@context-mode": true
   ```
   - Already installed: "context-mode 플러그인이 이미 설치되어 있습니다."
...
5. **Result message**:
   ...
   | 항목 | 상태 |
   |------|------|
   | context-mode 마켓플레이스 | ✅ 등록됨 |
   | context-mode 플러그인 | ✅ 설치됨 |

   Claude Code를 재시작하거나 /reload-plugins 로 적용하세요.
   적용 후 무거운 명령(테스트·git log·API 호출 등)이 자동으로 sandbox 라우팅됩니다.
   사용량 확인: /context-mode:ctx-stats
   ```
...
1. **Status check**: query all items at once via Bash:
   ...
   echo "CTXMODE_MP=$(grep -q '\"context-mode\"' ~/.claude/settings.json 2>/dev/null && echo 'REGISTERED' || echo 'NOT_REGISTERED')"
   echo "CTXMODE_PLUGIN=$(grep -q 'context-mode@context-mode' ~/.claude/settings.json 2>/dev/null && echo 'INSTALLED' || echo 'NOT_INSTALLED')"
   ```
...
2. **Install missing items sequentially**:
   ...
   **Step 0. Node.js** — if `NODE=NOT_INSTALLED`:
   ...
   d2. **context-mode** — if `CTXMODE_MP=NOT_REGISTERED` or `CTXMODE_PLUGIN=NOT_INSTALLED`:
      - Run the same procedure as the plugins install-context-mode action (marketplace registration + plugin enable)
...
3. **Result summary**: display as table:
   ...
   | context-mode 마켓플레이스 | ✅ 등록됨 / ⏭️ 이미 등록됨 |
   | context-mode 플러그인 | ✅ 설치됨 / ⏭️ 이미 설치됨 |
...
2. **Full reinstall** (run unconditionally for each item):
   ...
   d2. **context-mode**:
      - Run the plugins install-context-mode action procedure (marketplace registration + plugin enable)
