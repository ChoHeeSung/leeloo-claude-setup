...
4. **태스크 분해 (Sonnet Task)**: Plan의 변경 사항/구현 단계를 분석해 독립적 태스크로 분해하는 작업은 Sonnet 서브 에이전트에 위임한다.
...
```
아래 Plan 문서를 실행 가능한 태스크 리스트로 분해하라.

## Plan 본문
{plan_markdown}

## 분해 규칙
1. Plan의 "구현 단계" / "Step" / "변경 사항" 섹션을 우선 참조.
2. 각 태스크는 독립적으로 완료 가능한 단위여야 함 (다른 태스크 결과에 의존하지 않는 것이 이상적).
3. 각 태스크에 다음을 부여:
   - 번호 (1부터 순차)
   - 제목 (한 줄 요약, 50자 이내)
   - 설명 (제목 — 뒤에 붙이는 보충 설명, 파일 경로 포함 권장)
   - 참조: Plan 문서의 해당 섹션 (예: `§Step3`, `§Phase2`)
4. 과도하게 세분화하지 말 것 (하나의 파일 1줄 수정 같은 미세 단위 금지).
5. 순서가 중요한 의존성은 태스크 순서로 표현.

## 출력 형식 (JSON 배열만)
```json
[
  {"num": 1, "task": "태스크 제목 — 설명", "ref": "§Step1"},
  {"num": 2, "task": "...", "ref": "§Step2"}
]
```
...
**결과 검증 (메인 세션):**
- [ ] JSON 배열 파싱 성공
- [ ] 각 항목에 num, task, ref 필드 존재
- [ ] Plan의 주요 섹션이 누락 없이 반영됨
- [ ] 시작/종료/소요는 `-`로 초기화
- [ ] 상태는 모두 ⬜

**품질 미달 시 폴백:** 메인 세션(Opus)에서 직접 분해.
