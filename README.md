...
| **leeloo-its-util** | 1.0.0 | ITS 업무 자동화 유틸리티 모음 — 도면 분석, 시설물 관리, 데이터 변환 |
...
// ~/.claude/settings.json
{
  "enabledPlugins": [
    ...
    "/path/to/leeloo-claude-setup/leeloo-its-util"
  ]
}
```
...
- 환경 점검: `bash leeloo-its-util/scripts/check-env.sh` (자동 설치: `--fix`)
...
│   └── scripts/
├── leeloo-its-util/             # 플러그인 4: ITS 업무 자동화 유틸리티
│   ├── plugin.json
│   ├── CLAUDE.md
