...
| 플러그인 | 버전 | 설명 |
|---------|------|------|
| **leeloo-kit** | 3.0.0 | 사내 표준 AI 개발 키트 — 하네스 엔지니어링 기반 자동화 + 다중 검증 |
...
- **Back-Pressure + Auto Quality Check**: 소스 저장 시 자동 lint/typecheck. 성공은 침묵, 실패만 피드백
...
- **SessionStart** — Failure Memory 요약, 린터 미설치 감지, TODO 진행률
- **PreToolUse(Bash)** — 위험 명령 차단 (rm -rf, git push --force 등)
- **PostToolUse(Bash)** — Bash 실패 감지 → failure-log 기록
- **PostToolUse(Write|Edit)** — 도구 에러 감지 + 소스 저장 시 자동 품질 검사
- **PostToolUse(mcp_)** — MCP 도구 실패 감지
- **PostToolUse(Skill)** — 스킬 완료 후 다음 단계 안내
- **Stop** — Failure Memory Loop: 반복 실패 → 유형별 파일 + CLAUDE.md 기록
