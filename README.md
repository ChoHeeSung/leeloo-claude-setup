...
- **세션 라이프사이클**: SessionStart(이전 세션 복원) → PreCompact(컨텍스트 보존) → SessionEnd(세션 요약 저장)
- **MCP 헬스체크**: MCP 서버 장애 시 지수 백오프 + TTL 캐싱으로 자동 복구
...
- **PostToolUseFailure(mcp_)** — MCP 서버 헬스체크 (지수 백오프, TTL 캐싱)
- **PreCompact** — 컨텍스트 압축 시 Failure Memory + TODO 상태 보존 마커
- **Stop** — 누적 편집 파일 배치 lint/typecheck (30s 타임아웃)
- **SessionEnd** — 세션 요약 저장 + Failure Memory Loop 처리
