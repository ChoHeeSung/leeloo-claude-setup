---
name: cross-validate
description: "gemini-cli로 현재 plan을 교차검증합니다. Usage: /cross-validate [plan-file-path]"
license: MIT
---
...
설치 방법: npm install -g @anthropic-ai/gemini-cli 또는 https://github.com/google-gemini/gemini-cli 참고
```
...
인자가 없는 경우:
1. Glob 도구로 `~/.claude/plans/*.md` 패턴을 검색하세요 (`.review.md`로 끝나는 파일은 제외).
2. 파일이 없으면 다음 메시지를 출력하고 중단하세요:
   ```
   plan 파일을 찾을 수 없습니다. plan mode에서 plan을 먼저 작성하거나, 파일 경로를 직접 지정하세요.
   Usage: /cross-validate [plan-file-path]
   ```
...
gemini 응답을 **현재 작업 중인 프로젝트의 루트 디렉토리**에 `{plan파일명}.review.md`로 저장하세요.
예: plan 파일이 `~/.claude/plans/my-plan.md`이면 → `{프로젝트루트}/my-plan.review.md`
